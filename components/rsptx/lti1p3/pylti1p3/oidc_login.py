...
class OIDCLogin(t.Generic[REQ, TCONF, SES, COOK, RED]):
    ...
    async def _prepare_redirect_url(self, launch_url: str) -> str:
        if not launch_url:
        ...
        # validate request
        self._registration = await self.validate_oidc_login()
    ...
    async def _prepare_redirect(self, launch_url: str) -> Redirect[RED]:
        auth_login_return_url = await self._prepare_redirect_url(launch_url)
        return self.get_redirect(auth_login_return_url)

    async def redirect(self, launch_url: str, js_redirect: bool = False) -> RED:
        """
        ...
        if self._cookies_check:
            if not self._is_new_window_request():
                ...
                response = self.get_response(html)
                return response

        redirect_obj = await self._prepare_redirect(launch_url)
        if js_redirect:
            return redirect_obj.do_js_redirect()
        return redirect_obj.do_redirect()

    async def get_redirect_object(self, launch_url: str) -> Redirect[RED]:
        return await self._prepare_redirect(launch_url)
