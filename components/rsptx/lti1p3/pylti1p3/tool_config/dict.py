...
class ToolConfDict(ToolConfAbstract[Request]):
    ...
    async def _get_registration(self, iss: str, iss_conf: TIssConf) -> Registration:
        reg = Registration()
    ...
    async def _get_deployment(self, iss_conf: TIssConf, deployment_id: str):
        if deployment_id not in iss_conf["deployment_ids"]:
            return None
        d = Deployment()
        return d.set_deployment_id(deployment_id)

    async def find_registration_by_issuer(self, iss: str, *args, **kwargs):
    ...
    async def find_deployment(self, iss: str, deployment_id: str):
        iss_conf = await self.get_iss_config(iss)
        return await self._get_deployment(iss_conf, deployment_id)
    ...
    async def get_public_key(self, iss: str, client_id: t.Optional[str] = None):
        if self.check_iss_has_many_clients(iss):
            if not client_id:
    ...
    async def get_private_key(self, iss: str, client_id: t.Optional[str] = None):
        if self.check_iss_has_many_clients(iss):
            if not client_id:
    ...
    async def get_iss_config(self, iss: str, client_id: t.Optional[str] = None):
        if not self._config:
