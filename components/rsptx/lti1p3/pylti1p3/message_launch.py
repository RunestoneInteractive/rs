...
class MessageLaunch(t.Generic[REQ, TCONF, SES, COOK]):
    ...
    def __init__(
        ...
        """
        Do not call directly, use create() or from_cache() instead.
        """
        ...
        # Can disable iat verification to prevent clock skew problems from invalidating tokens
        # self._jwt_verify_options = {"verify_aud": False, "verify_iat": False}
    ...
    async def validate(self) -> "MessageLaunch":
        """
    ...
    def get_dls(self) -> TDeepLinkData:
        ...
        :return: TDeepLinkData
        """
        assert self._registration is not None, "Registration not yet set"
        deep_link_data = self._get_jwt_body().get(
    ...
    def get_context(self) -> TContextClaim:
        ...
        :return: dict  Returns the context claim from the current launch.
        """
        jwt_body = self._get_jwt_body()
        return jwt_body.get("https://purl.imsglobal.org/spec/lti/claim/context", None)
    ...
    async def fetch_public_key(self, key_set_url: str) -> TKeySet:
        cache_key = (
        ...
        with DisableSessionId(self._public_key_cache_data_storage):
            ...
            try:
                resp = await self._requests_session.get(key_set_url)
            except Exception as e:
                raise LtiException(
                    f"Error during fetch URL {key_set_url}: {str(e)}"
                ) from e
            try:
                public_key = await resp.json()
                if self._public_key_cache_data_storage:
    ...
    async def get_public_key(self) -> t.Tuple[str, str]:
        assert self._registration is not None, "Registration not yet set"
        ...
        if not public_key_set:
            ...
            if key_set_url.startswith(("http://", "https://")):
                public_key_set = await self.fetch_public_key(key_set_url)
    ...
    async def validate_registration(self) -> "MessageLaunch":
        iss = self.get_iss()
        jwt_body = self._get_jwt_body()
        ...
        if config.check_iss_has_one_client(iss):
            self._registration = await config.find_registration(
                iss, action=Action.MESSAGE_LAUNCH, request=req, jwt_body=jwt_body
            )
        else:
            self._registration = await config.find_registration_by_params(
                iss,
                client_id,
    ...
    async def validate_jwt_signature(self) -> "MessageLaunch":
        id_token = self._get_id_token()

        # Fetch public key object
        public_key, key_alg = await self.get_public_key()
    ...
    async def validate_deployment(self) -> "MessageLaunch":
        iss = self.get_iss()
        client_id = self.get_client_id()
        deployment_id = self._get_deployment_id()
        ...
        if tool_config.check_iss_has_one_client(iss):
            deployment = await tool_config.find_deployment(iss, deployment_id)
        else:
            deployment = await tool_config.find_deployment_by_params(
