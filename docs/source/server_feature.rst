...
.. note:: web2py has been removed

      The legacy web2py server was removed from the monorepo in August 2026.
      Its responsibilities now live on the FastAPI servers: authentication and
      the instructor interface on the admin server, book interaction on the
      book server, assignments and peer instruction on the assignment server.
      The spaced-practice feature was retired rather than ported.  If you need
      to read the old code, check out the ``legacy-web2py`` tag.
...
.. note:: dstart is for developing on ONE service at a time
      ...
      The ``dstart`` script is not for production.  It is only for development.  It is not secure, and it is designed to let you interact with one service at a time.  It does not redirect you to other services.   So for example if you ``dstart admin`` and try to go to a course it will fail.  But if you ``dstart admin`` and want to work on the login or instructor pages by just reloading, it will work great and will be much faster than rebuilding the container.
