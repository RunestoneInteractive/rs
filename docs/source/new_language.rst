...
Once you have your PreTeXt development environment working, test it before making any changes. In particular, start with a Runestone book that has active code within it, and change it to your new language. An easy example is the PreTeXt sample book; look in the file ``rune.xml``, which has a variety of examples. Within the ``<program>`` tag, change the ``language`` property to have your new language. Of course, this won't work yet, but this will help you see what you're looking to test. Once you have made that change, build the book at the command line::
