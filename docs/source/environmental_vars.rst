...
* ``WEB2PY_PRIVATE_KEY`` *(d)* - the secret used when hashing passwords.  Despite the name this is **still required**: web2py itself is gone, but every existing password in ``auth_user`` was hashed with this key, so changing or losing it locks every account out.  It should be a long random string -- generate one with ``openssl rand -base64 32`` -- and must be the same value in all of the services.
* ``SERVER_CONFIG`` *(d)* - this should be production, development, or test.  It is used to determine which database URL to use.
* ``WEB2PY_CONFIG`` *(d)* - should be the same value as ``SERVER_CONFIG``.  It is used to determine which database URL to use.  Another historical name that outlived web2py: the build tooling (``build_books``, ``build_mon``, ``buildptx``), the author server's worker, and ``rsmanage`` all still read it.
