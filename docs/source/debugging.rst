...
When I try to go to ``http://localhost`` I get redirected somewhere I did not expect, such as the Runestone landing page.  Any URL that no service claims is handled by the admin server's front door, which sends a signed-in reader to their current course and everyone else to ``settings.landing_page_url``.  So an unexpected redirect to the landing page usually means either that you are not logged in, or that the URL is simply wrong.  Check the ``admin`` log as described above.
