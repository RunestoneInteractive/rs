...
def insertAnswer(ans, sid, divid, info, ts, course):
    ...
    s = mcanswers.insert().values(
        timestamp=ts,
        div_id=divid,
        sid=sid,
        course_name=course,
        answer=responseMap.get(response, response),
        correct=iscorrect,
    )
    engine.execute(s)
...
s = (
    select([useinfo])
    .where(
        (useinfo.c.timestamp > datetime.datetime(2015, 1, 1))
        & (useinfo.c.event == "mChoice")
    )
    .order_by(useinfo.c.div_id, useinfo.c.sid, useinfo.c.id)
)
...
correctSeen = insertAnswer(
    None, currentSid, currentDiv, first["act"], first["timestamp"], first["course_id"]
)
    ...
    if row["div_id"] == currentDiv:
        if row["sid"] == currentSid:
            if not correctSeen:
                ...
                print(
                    "ignoring all answers after first correct for ",
                    currentDiv,
                    currentSid,
                )
        else:
        ...
        currentSid = row["sid"]
        correctSeen = insertAnswer(
            None, currentSid, currentDiv, row["act"], row["timestamp"], row["course_id"]
        )
