...
def run_webpack():
    ...
    p = subprocess.run(
        ["npm", "run", "build"], text=True, shell=IS_WINDOWS, capture_output=True
    )
...
def selenium_module_fixture(request):
    ...
    exit_status_success = (
        True if exit_status_success_mark is None else exit_status_success_mark.args[0]
    )
...
class ModuleFixture(unittest.TestCase):
    ...
    def setUpModule(self):
        ...
        p = subprocess.run(
            [sys.executable, "-m", "runestone", "build", "--all"],
            capture_output=True,
            text=True,
        )
