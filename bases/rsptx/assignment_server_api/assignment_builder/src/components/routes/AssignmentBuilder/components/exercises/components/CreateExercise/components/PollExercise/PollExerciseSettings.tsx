...
export interface PollExerciseSettings {
  name: string;
  author: string;
  topic: string;
  chapter: string;
...
export const PollExerciseSettings = ({
  onSettingsChange,
  initialData,
  showValidation = true
}: PollExerciseSettingsProps) => {
  const { chapters } = useExercisesSelector();

  // Initialize state with default values or initialData
  const [settings, setSettings] = useState<PollExerciseSettings>(() => ({
  ...
  const updateSetting = useCallback((field: keyof PollExerciseSettings, value: any) => {
    setSettings((prev) => ({
      ...prev,
      [field]: value
    }));
