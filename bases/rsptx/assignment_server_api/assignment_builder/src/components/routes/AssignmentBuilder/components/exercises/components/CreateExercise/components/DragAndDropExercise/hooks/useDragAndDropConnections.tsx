...
import { BlockConnection, DragAndDropData } from "../types";
...
export const useDragAndDropConnections = ({
  ...
  const handleStartConnection = useCallback(
    (sourceId: string) => {
      const hasConnection = (formData.connections || []).some((conn) => conn.sourceId === sourceId);

      if (hasConnection) {
        toastRef.current?.show({
          severity: "warn",
          summary: "Already Connected",
          detail: "This block already has a connection",
          life: 3000
        });
        return;
      }
    ...
    [formData.connections, getBlockPosition, toastRef]
  );
  ...
  const handleCompleteConnection = useCallback(
    (targetId: string) => {
      ...
      const targetHasConnection = (formData.connections || []).some(
      ...
      if (targetHasConnection) {
        toastRef.current?.show({
          severity: "warn",
          summary: "Target Connected",
          detail: "This target already has a connection",
          life: 3000
        });
      } else {
        const newConnection: BlockConnection = {
          id: `conn-${activeSource}-${targetId}`,
          sourceId: activeSource,
          targetId
        };

        updateFormData("connections", [...(formData.connections || []), newConnection]);
      }

      setActiveSource(null);
    },
    [activeSource, formData.connections, updateFormData, toastRef]
  );
  ...
  const handleDeleteConnection = useCallback(
    ...
    [formData.connections, updateFormData]
  );
