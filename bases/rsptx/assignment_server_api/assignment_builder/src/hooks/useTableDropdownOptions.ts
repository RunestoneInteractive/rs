...
export const useTableDropdownOptions = (
  ...
  return {
    autograde: autoGradeOptions.filter((op) => op.supported_question_types.includes(question_type)),
    which_to_grade: whichToGradeOptions.filter((op) =>
      op.supported_question_types.includes(question_type)
    )
  };
};
